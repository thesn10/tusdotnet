...
namespace tusdotnet.Storage.Tus2
{
    ...
    public class Tus2DiskStorage : Tus2Storage
    {
        ...
        public override Task CreateFile(string uploadToken, IDictionary<string, Metadata> metadata)
        {
            uploadToken = CleanUploadToken(uploadToken);

            if (metadata != null)
                File.WriteAllText(_diskPathHelper.MetadataFilePath(uploadToken), GetMetadataString(metadata));

            File.Create(_diskPathHelper.DataFilePath(uploadToken)).Dispose();
            return Task.CompletedTask;
        }

        public override async Task WriteData(string uploadToken, PipeReader reader, CancellationToken cancellationToken)
        {
            uploadToken = CleanUploadToken(uploadToken);
