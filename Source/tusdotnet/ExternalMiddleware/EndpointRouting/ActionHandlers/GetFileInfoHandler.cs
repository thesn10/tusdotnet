...
namespace tusdotnet.ActionHandlers
{
    ...
    */
    ...
    internal class GetFileInfoHandler : IntentHandler
    {
        ...
        public GetFileInfoHandler(ContextAdapter context, ITusStore store)
            : base(context, store, IntentType.GetFileInfo, LockType.NoLock)
        {
        }
        ...
        internal override async Task Invoke()
        {
            ...
            SetUploadLengthHeader(Context, Store, uploadLength);
            ...
            if (Store is ITusConcatenationStore tusConcatStore)
            {
        ...
        {
            if (Store is ITusCreationStore tusCreationStore)
                return tusCreationStore.GetUploadMetadataAsync(Request.FileId, context.CancellationToken);

            return Task.FromResult<string>(null);
        }

        private static void SetUploadLengthHeader(ContextAdapter context, ITusStore store, long? uploadLength)
        {
            if (uploadLength != null)
            {
                context.Response.SetHeader(HeaderConstants.UploadLength, uploadLength.Value.ToString());
            }
            else if (store is ITusCreationDeferLengthStore)
            {
