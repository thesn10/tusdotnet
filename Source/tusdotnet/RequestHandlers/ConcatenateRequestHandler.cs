...
namespace tusdotnet.RequestHandlers
{
    ...
    *   NOTE: The above is only applicable for partial files as final files cannot be patched.
    ...
    internal class ConcatenateRequestHandler : RequestHandler
    {
        ...
        public override RequestRequirement[] Requires => BuildListOfRequirements();
        ...
        private RequestRequirement[] BuildListOfRequirements()
        {
            ...
            if (_isPartialFile)
            {
                requirements.Add(new UploadLengthForCreateFileAndConcatenateFiles(EndpointOptions.MaxAllowedUploadSizeInBytes));
            }

            requirements.Add(new UploadMetadata(metadata => _metadataFromRequirement = metadata, EndpointOptions.MetadataParsingStrategy));
